...
#
# (b) Uses unweighted LCA solutions to estimate weighted group membership
#
# (c) 
#
# (d) 
...
### employment contract
emp_contracta_lca_final <- emp_contracta_lca_final %>% 
  left_join(weight_spine_a)

emp_contractb_lca_final <- emp_contractb_lca_final %>% 
...
multi_empb_lca_final <- multi_empb_lca_final %>% 
  left_join(weight_spine_b)
...
svy_emp_contract_a <- svydesign(id=~psu, strata=~strata,
                            weights=~indinub_xw, data=emp_contracta_lca_final)
...
svy_multi_emp_a <- svydesign(id=~psu, strata=~strata,
                                weights=~indinub_xw, data=multi_empa_lca_final)

svy_multi_emp_b <- svydesign(id=~psu, strata=~strata,
                                weights=~indinui_xw, data=multi_empb_lca_final)


################################################################################
#####             weighted estimates of latent class membership            #####
################################################################################
...
emp_contract_a <- data.frame(svymean(~emp_contract_class, svy_emp_contract_a))
...
## calculate proportions
emp_contract_b <- data.frame(svymean(~emp_contract_class, svy_emp_contract_b))
...
## calculate totals
emp_contract2_b <- data.frame(svytotal(~emp_contract_class, svy_emp_contract_b))
emp_contract2_b <- emp_contract2_b %>% dplyr::select(-SE)
emp_contract2_b <- cbind(rownames(emp_contract2_b),emp_contract2_b, row.names=NULL)
emp_contract2_b$`rownames(emp_contract2_b)` <- str_replace(emp_contract2_b$`rownames(emp_contract2_b)`, "emp_contract_class","")
emp_contract2_b <- emp_contract2_b %>% rename(measure = `rownames(emp_contract2_b)`)
emp_contract2_b$total <- as.integer(emp_contract2_b$total)

## join together and format
emp_contract_b <- emp_contract_b %>%
  left_join(emp_contract2_b) %>% 
  mutate(est = mean*100,
         var="Employment contract class",
         wv_n=10) %>% 
  rename(n=total) %>% 
...
## calculate proportions
multi_emp_a <- data.frame(svymean(~multi_emp_class, svy_multi_emp_a))
...
## calculate totals
multi_emp2_a <- data.frame(svytotal(~multi_emp_class, svy_multi_emp_a))
multi_emp2_a <- multi_emp2_a %>% dplyr::select(-SE)
multi_emp2_a <- cbind(rownames(multi_emp2_a),multi_emp2_a, row.names=NULL)
multi_emp2_a$`rownames(multi_emp2_a)` <- str_replace(multi_emp2_a$`rownames(multi_emp2_a)`, "multi_emp_class","")
multi_emp2_a <- multi_emp2_a %>% rename(measure = `rownames(multi_emp2_a)`)
multi_emp2_a$total <- as.integer(multi_emp2_a$total)

## join together and format
multi_emp_a <- multi_emp_a %>%
  left_join(multi_emp2_a) %>% 
  mutate(est = mean*100,
         var="Multiple employment class",
         wv_n=6) %>% 
  rename(n=total) %>% 
  dplyr::select(wv_n, var, measure, n, est, se) %>% 
...
## calculate proportions
multi_emp_b <- data.frame(svymean(~multi_emp_class, svy_multi_emp_b))
...
## calculate totals
multi_emp2_b <- data.frame(svytotal(~multi_emp_class, svy_multi_emp_b))
multi_emp2_b <- multi_emp2_b %>% dplyr::select(-SE)
multi_emp2_b <- cbind(rownames(multi_emp2_b),multi_emp2_b, row.names=NULL)
multi_emp2_b$`rownames(multi_emp2_b)` <- str_replace(multi_emp2_b$`rownames(multi_emp2_b)`, "multi_emp_class","")
multi_emp2_b <- multi_emp2_b %>% rename(measure = `rownames(multi_emp2_b)`)
multi_emp2_b$total <- as.integer(multi_emp2_b$total)

## join together and format
multi_emp_b <- multi_emp_b %>%
  left_join(multi_emp2_b) %>% 
  mutate(est = mean*100,
         var="Multiple employment class",
         wv_n=10) %>% 
  rename(n=total) %>% 
...
#
#### employment contract 
#dfas1a_seq_wide <- readRDS("./working_data/weighted/dfas1a_seq_wide.rds")
#dfas1b_seq_wide <- readRDS("./working_data/weighted/dfas1b_seq_wide.rds")
#
#### broken employment spells
#dfas1a_seq_wide2 <- readRDS("./working_data/weighted/dfas1a_seq_wide2.rds")
#dfas1b_seq_wide2 <- readRDS("./working_data/weighted/dfas1b_seq_wide2.rds")
#
#### multiple jobs
#dfas1a_seq_wide3 <- readRDS("./working_data/weighted/dfas1a_seq_wide3.rds")
#dfas1b_seq_wide3 <- readRDS("./working_data/weighted/dfas1b_seq_wide3.rds")
#
##### end-point data ---------------
#dfas1a_end <- readRDS("./working_data/dfas1a_end.rds")
#dfas1b_end <- readRDS("./working_data/dfas1b_end.rds")
#
#### drop unsed factor levels
...
#covar_a <- dfas1a_end %>% 
#  dplyr::select(pidp, sex_dv, age_dv, hiqual_dv, gor_dv)
#
#covar_b <- dfas1b_end %>% 
#  dplyr::select(pidp, sex_dv, age_dv, hiqual_dv, gor_dv)
#
##### join on covariates to wide seq data -------
#
#### employment contract 
...
#
#### broken employment spells
#dfas1a_seq_wide2 <- dfas1a_seq_wide %>% 
#  left_join(covar_a)
#
#dfas1b_seq_wide2 <- dfas1b_seq_wide %>% 
#  left_join(covar_b)
#
#### multiple jobs
#dfas1a_seq_wide3 <- dfas1a_seq_wide %>% 
#  left_join(covar_a)
#
#dfas1b_seq_wide3 <- dfas1b_seq_wide %>% 
#  left_join(covar_b)
#
#################################################################################
...
#
#f <- cbind(wv_3, wv_4,wv_5, wv_6) ~ sex_dv+age_dv+hiqual_dv+gor_dv
#f2 <- cbind(wv_7, wv_8,wv_9, wv_10) ~ sex_dv+age_dv+hiqual_dv+gor_dv
#
#### covariates, 1 class --------------------
...
#
## convert covariates into odds ratios by exponentiating coefficients
#empcontract_lca3a_or <- data.frame(empcontract_lca3a$coeff) %>% 
#  mutate(or1 = exp(X1),
#         or2 = exp(X2))
#
#empcontract_lca3b_or <- data.frame(empcontract_lca3b$coeff) %>% 
#  mutate(or1 = exp(X1),
#         or2 = exp(X2))
#
### here <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#
## bic elbow plot
#tiff("./output/weighted/empcontracta_lca_elbow.tiff", width = 400, height = 400)
...
#dev.off()
#
#tiff("./output/weighted/empcontractb_lca_elbow.tiff", width = 400, height = 400)
...
#
### save fit_stats
#write.csv(empcontracta_lca_fit_stats, "./output/weighted/empcontracta_lca_fit_stats.csv")
#
#write.csv(empcontractb_lca_fit_stats, "./output/weighted/empcontractb_lca_fit_stats.csv")
#
#
##### ---------------------------------------------------------------------------
##### Broken employment spells
...
#
## bic elbow plot
#tiff("./output/weighted/empspellsa_lca_elbow.tiff", width = 400, height = 400)
...
#dev.off()
#
#tiff("./output/weighted/empspellsb_lca_elbow.tiff", width = 400, height = 400)
...
#
### save fit_stats
#write.csv(empspellsa_lca_fit_stats, "./output/weighted/empspellsa_lca_fit_stats.csv")
#
#write.csv(empspellsb_lca_fit_stats, "./output/weighted/empspellsb_lca_fit_stats.csv")
#
##### ---------------------------------------------------------------------------
##### Multiple employment
##### ---------------------------------------------------------------------------
...
#
## bic elbow plot
#tiff("./output/weighted/multi_empa_lca_elbow.tiff", width = 400, height = 400)
...
#dev.off()
#
#tiff("./output/weighted/multi_empb_lca_elbow.tiff", width = 400, height = 400)
...
#
### save fit_stats
#write.csv(multi_empa_lca_fit_stats, "./output/weighted/multi_empa_lca_fit_stats.csv")
#
#write.csv(multi_empb_lca_fit_stats, "./output/weighted/multi_empb_lca_fit_stats.csv")
#
#################################################################################
...
#                                                                "CHECK"))))))
#
#write_rds(emp_contractb_lca_final, "./working_data/weighted/emp_contractb_lca_final.rds")
#
#
...
#  dplyr::select(pidp,emp_contract_class)
#
#write_rds(emp_contracta_5class_spine, "./working_data/weighted/emp_contracta_5class_spine.rds")
#write_rds(emp_contractb_5class_spine, "./working_data/weighted/emp_contractb_5class_spine.rds")
#
...
#saveRDS(emp_contracta_lca_final_model, "./output/weighted/emp_contracta_lca_final_model.rds")
#
#emp_contractb_lca_final_model <- poLCA(f2, dfas1b_seq_wide, nclass = 5, maxiter = 8000, probs.start=new.probs.start.b, 
#                                       graphs = FALSE, na.rm = FALSE)
#saveRDS(emp_contractb_lca_final_model, "./output/weighted/emp_contractb_lca_final_model.rds")
#
## save plot
#tiff("./output/weighted/emp_contracta_lca_final.tiff", width = 400, height = 400)
#poLCA(f, dfas1a_seq_wide, nclass = 5, maxiter = 8000, probs.start=probs.start, 
#      graphs = TRUE, na.rm = FALSE)
#dev.off()
#
#tiff("./output/weighted/emp_contractb_lca_final.tiff", width = 400, height = 400)
...
##  dplyr::select(pidp,emp_spells_class)
##
##write_rds(emp_spellsa_5class_spine, "./working_data/weighted/emp_spellsa_5class_spine.rds")
##write_rds(emp_spellsb_5class_spine, "./working_data/weighted/emp_spellsb_5class_spine.rds")
##
...
##                                       graphs = FALSE, na.rm = FALSE)
##saveRDS(emp_spellsa_lca_final_model, "./output/weighted/emp_spellsa_lca_final_model.rds")
##
##emp_spellsb_lca_final_model <- poLCA(f2, dfas1b_seq_wide, nclass = 5, maxiter = 8000, probs.start=probs.start.b, 
##                                       graphs = FALSE, na.rm = FALSE)
##saveRDS(emp_spellsb_lca_final_model, "./output/weighted/emp_spellsb_lca_final_model.rds")
##
### save plot
##tiff("./output/weighted/emp_spellsa_lca_final.tiff", width = 400, height = 400)
##poLCA(f, dfas1a_seq_wide2, nclass = 5, maxiter = 8000, probs.start=probs.start, 
##      graphs = TRUE, na.rm = FALSE)
##dev.off()
##
##tiff("./output/weighted/emp_spellsb_lca_final.tiff", width = 400, height = 400)
...
#                                                                "CHECK"))))))
#
#write_rds(multi_empa_lca_final, "./working_data/weighted/multi_empa_lca_final.rds")
#
#
...
#                                                             "CHECK"))))))
#
#write_rds(multi_empb_lca_final, "./working_data/weighted/multi_empb_lca_final.rds")
#
#
### create spine for allocating classes back onto survey data
...
#  dplyr::select(pidp,multi_emp_class)
#
#write_rds(multi_empa_5class_spine, "./working_data/weighted/multi_empa_5class_spine.rds")
#write_rds(multi_empb_5class_spine, "./working_data/weighted/multi_empb_5class_spine.rds")
#
...
#                                       graphs = FALSE, na.rm = FALSE)
#saveRDS(multi_empa_lca_final_model, "./output/weighted/multi_empa_lca_final_model.rds")
#
#multi_empb_lca_final_model <- poLCA(f2, dfas1b_seq_wide3, nclass = 5, maxiter = 8000, probs.start=new.probs.start.b, 
#                                       graphs = FALSE, na.rm = FALSE)
#saveRDS(multi_empb_lca_final_model, "./output/weighted/multi_empb_lca_final_model.rds")
#
## save plot
#tiff("./output/weighted/multi_empa_lca_final.tiff", width = 400, height = 400)
#poLCA(f, dfas1a_seq_wide3, nclass = 5, maxiter = 8000, probs.start=new.probs.start.a, 
#      graphs = TRUE, na.rm = FALSE)
#dev.off()
#
#tiff("./output/weighted/multi_empb_lca_final.tiff", width = 400, height = 400)
